...
/**
 * Check if switch is set correctly on encoder test current code with fudge factor to keep encoder
 * in range. Maybe add periodic thing that checks if encoder has gone past 0 to overflow, and then
 * autocorrects itself.
 */
public class EndEffector extends SubsystemBase {
  private final CANSparkMax neo =
      new CANSparkMax(
          Constants.EndEffectorConstants.neoID, MotorType.kBrushless); // check if neo is brushed
  private final CANSparkMax snowblower =
      new CANSparkMax(Constants.EndEffectorConstants.snowblowerID, MotorType.kBrushed);

  // private final SparkMaxAbsoluteEncoder absoluteEncoder =
  // snowblower.getAbsoluteEncoder(SparkMaxAbsoluteEncoder.Type.kDutyCycle);
  ...
  public void periodic() {
    ...
    /** if(snowblowerEncoder.getPosition() >= 6){ snowblowerEncoder.setPosition(2.667); } */
  ...
  public double getWristAngle() {
    // getPosition() returns rotations of motor, which is identical to rotations of encoder since
    // mirrored gearing
    // Rotations of motor * gear ratio (1/2.667) = rotations of wrist
    // rotations of wrist * 360 = degrees

    /**
     * Encoder count seems to max out at 4,500,000 4500000 / 8192 = 549.31640625 Hence the encoder
     * maxes out number of revolutions at 549.31640625 If set the initial position to 2.667, and
     * then subtract 2.667 from all positions for the return number, we can get numbers between
     * (2.667,-2.667), so we divide by 2.667 to get from rotations of the motor to rotations of
     * wrist
     */
    /* spotless:off
    if (angle > 360) {
   ...
   *
   * @param speed - a double between 0 and 1, representing the speed that we want the motor to run
   *     at
   */
  ...
  /**
   * Turns the wrist mechanism at the input speed, but ideally will automatically stop it if the
   * wrist has moved beyond its limits.
   *
   * @param speed - a double between 0 and 1, representing the speed that we want the wrist to turn
   *     at
   */
  public void turnWrist(double speed) {
    /* spotless:off
    if (snowblowerEncoder.getPosition() >= 120
        && speed > 0) { // double check that angle and speed match up correctly
      speed = 0;
  ...
  public PIDCommand turnWristToAngle(double goalAngle) {
    ...
    return new PIDCommand(
        ...
        () -> getWristAngle(),
        goalAngle,
        output -> turnWristSetpoints(output),
        this);
  }
  ...
  public SequentialCommandGroup intakeCone(boolean isForward) {
    if (isForward) {
      return new SequentialCommandGroup(
          turnWristToAngle(-90), new InstantCommand(() -> runIntake(0.5)));
    } else {
      return new SequentialCommandGroup(
          turnWristToAngle(90), new InstantCommand(() -> runIntake(-0.5)));
    }
  ...
  /**
   * If the cone is pointed forwards (and wrist pointed backwards), this command flips the wrist
   * forwards to put the cone upright. Otherwise, it flips the wrist backwards to put a reversed
   * cone upright.
   *
   * @return ProxyCommand
   */
  ...
  /**
   * If the wrist is pointed forwards, it reverses the intake to release a cone, and if the wrist is
   * pointed backwards, it runs the intake forwards to release.
   *
