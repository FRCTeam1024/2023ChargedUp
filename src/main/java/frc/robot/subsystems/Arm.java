...
public class Arm extends SubsystemBase {
  ...
  public Arm() {
    ...
    leftArmMotor.setInverted(
        false); // inverting the left side motor for now - may need to be switched later
  ...
  /**
   * @return the angle of the arm in degrees based on the crank angle and limited to the max and min
   *     transmission angles.
   */
  public double getArmAngle() {
    return crankToArm(
        MathUtil.clamp(getCrankAngle(), ArmConstants.minCrankAngle, ArmConstants.maxCrankAngle));
  }
  ...
  public double crankToArm(double crank) {
    ...
    return Math.toDegrees(
            Math.asin(
                (Math.pow(ArmConstants.R2, 2) - Math.pow(ArmConstants.R3, 2) - (A * A + B * B))
                    / (2 * ArmConstants.R3 * Math.sqrt(A * A + B * B))))
  ...
  public double getCrankAngle() {
    return rightArmMotor.getSelectedSensorPosition() * 360 / (2048 * ArmConstants.armGearRatio)
        + ArmConstants.minCrankAngle;
    //     need to see left vs right motors for sensor        double check sensor units value
    //     could check if the inversion of the left motor allows us to use an average of the two
    // values
  }
  ...
  public double getCrankSpeed() {
    return rightArmMotor.getSelectedSensorVelocity()
        * 360
        * 10
        / (2048 * ArmConstants.armGearRatio);
  }
  ...
  /**
   * A reverse kinematics method based on the arm geometry. All angles are with respect to 0 degrees
   * being considered horizontal forward and proceed CCW looking at the RH side
   *
   * @param armAngle the angle of the arm (degrees) for which the corresponding crank angle is
   *     desired
  ...
  private double armToCrank(double armAngle) {
    ...
    double C =
        ArmConstants.X3 + ArmConstants.R3 * Math.cos(Math.toRadians(armAngle + ArmConstants.PHI));
    double D =
        ArmConstants.Y3 + ArmConstants.R3 * Math.sin(Math.toRadians(armAngle + ArmConstants.PHI));

    double theAngle =
        Math.toDegrees(
                Math.asin(
                        (Math.pow(ArmConstants.R1, 2)
                            - Math.pow(ArmConstants.R2, 2)
                            + C * C
                            + D * D))
                    / (2 * ArmConstants.R1 * Math.sqrt(C * C + D * D)))
  ...
  private double getRodAngle() {
    ...
    double X2 =
        ArmConstants.X3
            + ArmConstants.R3 * Math.cos(Math.toRadians(getArmAngle() + ArmConstants.PHI));
    double Y2 =
        ArmConstants.Y3
            + ArmConstants.R3 * Math.sin(Math.toRadians(getArmAngle() + ArmConstants.PHI));

    return Math.toDegrees(Math.atan((Y2 - Y1) / (X2 - X1)));
  }

  /**
   * This method should use a trapezoid motion profile to govern the arm angle from the current
   * position to the goal angle. Use the armToCrank method to determine the crank angle at each time
   * step, then use a proportional gain and a state based feedforward gain that accounts for the
   * position of the arm.
   *
  ...
  public Command moveTo(double goalAngle) {
    ...
    TrapezoidProfile.Constraints constraints =
        new TrapezoidProfile.Constraints(
            90, 90); // We'll work in degrees here since the arm angle methods return degrees
    ...
    goalAngle =
        MathUtil.clamp(
            goalAngle,
            crankToArm(ArmConstants.minCrankAngle),
            crankToArm(ArmConstants.maxCrankAngle));

    return new ProfiledPIDCommand(
            crankController,
            () -> getArmAngle(),
            goalAngle,
            (output, setpoint) -> move(output, setpoint),
            this)
  ...
  public Command moveToAuto(double goalAngle) {
    ...
    TrapezoidProfile.Constraints constraints =
        new TrapezoidProfile.Constraints(
            120, 140); // We'll work in degrees here since the arm angle methods return degrees
    ProfiledPIDController crankController = new ProfiledPIDController(.5, 0, 0, constraints);
  ...
  public Command calMove() {
    ...
    return new PIDCommand(
            crankCalController, () -> getCrankSpeed(), -60.0, (output) -> simpleMove(output), this)
        .withInterruptBehavior(InterruptionBehavior.kCancelSelf);
  }
  ...
  /**
   * This method returns true when the arm crank is triggered indicating the arm is at its low end
   * setpoint.
   *
