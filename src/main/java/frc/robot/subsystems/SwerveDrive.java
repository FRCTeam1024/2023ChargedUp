...
public class SwerveDrive extends SubsystemBase {
  ...
  public SwerveDrive() {
    ...
    m_poseEstimator =
        new SwerveDrivePoseEstimator(
            m_kinematics, new Rotation2d(Math.PI), modulePositions, new Pose2d());
  }
        ...
        m_kinematics.toSwerveModuleStates(
            fieldRelative
                ? ChassisSpeeds.fromFieldRelativeSpeeds(
                    xSpeed, ySpeed, rot, m_odometry.getPoseMeters().getRotation())
  ...
  public void visionEstimatedPose() {
    ...
    m_poseEstimator.addVisionMeasurement(pair.getFirst(), pair.getSecond());
  }
      ...
      targets.sort(
          new Comparator<PhotonTrackedTarget>() {
            ...
            public int compare(PhotonTrackedTarget a, PhotonTrackedTarget b) {
              ...
              double dist1 =
                  Math.sqrt(Math.pow(translate1.getX(), 2) + Math.pow(translate1.getY(), 2));
              Translation3d translate2 = b.getBestCameraToTarget().getTranslation();
              double dist2 =
                  Math.sqrt(Math.pow(translate2.getX(), 2) + Math.pow(translate2.getY(), 2));
              if (dist2 > dist1) {
                return -1;
              } else if (dist1 > dist2) {
                return 1;
        ...
        targetPose =
            robotPose.plus(
                new Transform2d(
                    new Translation2d(
                        camToTarget.getX() - 1,
                        -camToTarget.getY()), // double check what needs negated
                    new Rotation2d(camToTarget.getRotation().getZ())));
      } else {
  ...
  public Command followVisionTrajectory() {
    ...
    return new SequentialCommandGroup(
        basicFirstTrajectory(path), new AutoTurn(this, 0), new InstantCommand(() -> this.stop()));
  }
