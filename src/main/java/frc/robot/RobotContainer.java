...
public class RobotContainer {
  ...
  // Default Commands
  private final DriveWithJoysticks driveWithController =
      new DriveWithJoysticks(drivetrain, driverController, .58, arm);
  private final TurnWristWithJoysticks turnWristWithJoysticks =
      new TurnWristWithJoysticks(endEffector, operatorController);
  ...
  /**
   * List any PID subsystems here so that they get disabled when the robot is disabled and integral
   * error doesn't accumulate. Usually doesn't matter since we often don't use integral gain but
   * just in case.
   */
  ...
  private void configureButtonBindings() {
    ...
    driverController.leftTrigger.whileTrue(
        new DriveWithJoysticks(drivetrain, driverController, 0.35, arm));
    driverController.leftBumper.whileTrue(
        new DriveWithJoysticks(drivetrain, driverController, 1, arm));
    ...
    operatorController.dPadUp.whileTrue(
        new ProxyCommand(
            () -> arm.moveTo(ArmConstants.highLevel))); // Use moveTo(ArmConstant.MaxArmAngle)
    operatorController.dPadDown.whileTrue(
        new ProxyCommand(
            () -> arm.moveTo(ArmConstants.stowLevel))); // Use moveTo(ArmConstants.MinArmAngle)
    ...
    // need to test and see if these should be instantcommands or proxycommands, as well as if we
    // need an automatic stop after movement
    /*
     ...
     * operatorController.dPadRight.whileTrue(new InstantCommand(() ->
     * endEffector.turnWrist(-0.6))); operatorController.dPadRight.onFalse(new InstantCommand(() ->
     * endEffector.stop()));
     */

    // if no more reverse cones, change the below angle to 120 or something higher
    operatorController.leftTrigger.whileTrue(
        new ProxyCommand(() -> endEffector.turnWristToAngle(95)));
    operatorController.leftBumper.whileTrue(
        new ProxyCommand(() -> endEffector.turnWristToAngle(-75)));
  ...
  private void configureDashboard() {
    ...
    diagnosticsTab.add("DrivetrainCommand", drivetrain).withSize(2, 1).withPosition(8, 0);
    ...
    // m_AutoChooser.addOption("C-OuterRoute-Charge", new ProxyCommand(() ->
    // C_OuterRoute_Charge()));
    ...
    // m_AutoChooser.addOption("C-InnerRoute-Charge", new ProxyCommand(() ->
    // C_InnerRoute_Charge()));
    ...
    m_AutoChooser.addOption(
        "OuterConesCharge - Testing only", new ProxyCommand(() -> OuterConesCharge()));
    ...
    driverTab.add("Auto Mode", m_AutoChooser).withSize(3, 1).withPosition(0, 0);

    // Adds the limelight camera feed
    driverTab
        .addCamera("limelight", "OV5647", "http://photonvision.local:1182/stream.mjpg")
        .withSize(3, 3)
        .withPosition(0, 1);

    driverTab
        .add("Arm Camera", arm.getFeed())
        .withSize(4, 4)
        .withPosition(3, 0)
        .withProperties(Map.of("ROTATION", "HALF"));

    driverTab.addNumber("Arm Angle", () -> arm.getArmAngle()).withSize(1, 1).withPosition(7, 0);

    driverTab
        .addNumber("Raw Wrist Angle", () -> endEffector.getRawWristAngle())
        .withSize(1, 1)
        .withPosition(7, 1);

    driverTab
        .addNumber("Wrist Angle", () -> endEffector.getWristAngle())
        .withSize(1, 1)
        .withPosition(7, 2);

    /*
     * driverTab.addNumber("AprilTag ID", () ->
     * drivetrain.getCamera().getBestTarget().getFiducialId()) .withSize(1,1) .withPosition(0,2);
     *
     * <p>driverTab.addBoolean("AprilTag targets", () -> drivetrain.getCamera().hasTargets())
     * .withSize(1,1) .withPosition(0,3);
     */
    diagnosticsTab
        .addNumber("Robot Yaw", () -> drivetrain.getYawDegrees())
        .withSize(1, 1)
        .withPosition(0, 0);

    diagnosticsTab
        .addNumber("Pigeon Heading", () -> drivetrain.getPigeonHeading())
        .withSize(1, 1)
        .withPosition(0, 1);

    diagnosticsTab
        .addNumber("Pigeon Yaw", () -> drivetrain.getPigeonYaw())
        .withSize(1, 1)
        .withPosition(0, 2);

    // Swerve module angles A-D
    diagnosticsTab
        .addNumber("SwerveModule A Angle", () -> drivetrain.getAngleRad(1))
        .withSize(1, 1)
        .withPosition(5, 0);

    diagnosticsTab
        .addNumber("SwerveModule B Angle", () -> drivetrain.getAngleRad(2))
        .withSize(1, 1)
        .withPosition(6, 0);

    diagnosticsTab
        .addNumber("SwerveModule C Angle", () -> drivetrain.getAngleRad(3))
        .withSize(1, 1)
        .withPosition(5, 1);

    diagnosticsTab
        .addNumber("SwerveModule D Angle", () -> drivetrain.getAngleRad(4))
        .withSize(1, 1)
        .withPosition(6, 1);

    diagnosticsTab
        .addNumber("Neo Temperature", () -> endEffector.getTemperature())
        .withSize(1, 1)
        .withPosition(3, 2);
    /*
    ...
    */
    ...
    diagnosticsTab
        .addBoolean("Is Practice Bot", () -> Constants.PracticeBot)
        .withSize(1, 1)
        .withPosition(4, 1);

    diagnosticsTab
        .addBoolean("Is Comp Bot", () -> Constants.CompBot)
        .withSize(1, 1)
        .withPosition(4, 2);
    ...
    diagnosticsTab
        .addBoolean("Arm At Limit", () -> arm.atCrankLimit())
        .withSize(1, 1)
        .withPosition(4, 0);
    ...
    /*
     * diagnosticsTab.addNumber("Robot Yaw", () -> drivetrain.getYawDegrees()) .withSize(1,1)
     * .withPosition(0,0);
     *
     * <p>diagnosticsTab.addNumber("X-Position", () -> drivetrain.getPose().getX()) .withSize(1,1)
     * .withPosition(1,0);
     *
     * <p>//putting the field in in case we want to have that visualization on shuffleboard
     * diagnosticsTab.add(field) .withSize(4,3) .withPosition(1,1);
     *
     * <p>diagnosticsTab.addString("Vision Estimated Pose", () -> drivetrain.getPose().toString())
     * .withSize(4,1) .withPosition(0,4);
     *
     * <p>diagnosticsTab.add("AutoAlignAprilTag", new AutoAlignAprilTag(drivetrain,
     * drivetrain.getCamera())) .withSize(2,1) .withPosition(0,2);
     *
     * <p>diagnosticsTab.addNumber("Target Yaw", () -> drivetrain.getTargetYaw()) .withSize(1,1)
     * .withPosition(0,3);
     *
     * <p>diagnosticsTab.add("AutoMoveToAprilTag", new AutoMoveToAprilTag(drivetrain,
     * drivetrain.getCamera()).move()) .withSize(2,1) .withPosition(7, 0);
     *
     * <p>diagnosticsTab.add("AutoBalance", new AutoBalance(drivetrain)) .withSize(2,1)
     * .withPosition(7,2);
     *
     * <p>diagnosticsTab.addString("Pose", () -> drivetrain.getPose().toString()) .withSize(2,1)
     * .withPosition(7,1);
     */
    diagnosticsTab
        .addNumber("CrankAngle", () -> arm.getCrankAngle())
        .withSize(1, 1)
        .withPosition(1, 1);

    diagnosticsTab
        .addNumber("GoalVelocity", () -> arm.getGoalVelocity())
        .withSize(1, 1)
        .withPosition(3, 1);

    diagnosticsTab
        .addNumber("ArmVoltage", () -> arm.getVoltage())
        .withSize(1, 1)
        .withPosition(5, 1);

    diagnosticsTab
        .addNumber("RobotPitch", () -> drivetrain.getPitch())
        .withSize(1, 1)
        .withPosition(7, 1);

    diagnosticsTab
        .addNumber("RobotRoll", () -> drivetrain.getRoll())
        .withSize(1, 1)
        .withPosition(8, 1);

    diagnosticsTab
        .addNumber("Neo Voltage", () -> endEffector.getNeoVoltage())
        .withSize(1, 1)
        .withPosition(4, 3);

    /*
     * diagnosticsTab.addNumber("Charging Station Angle", () -> drivetrain.getChargeStationAngle())
     * .withSize(1,1) .withPosition(7,3);
     */
    driverTab
        .addNumber("Battery Voltage", () -> RobotController.getBatteryVoltage())
        .withSize(1, 1)
        .withPosition(7, 3);

    diagnosticsTab
        .addString("Pose:", () -> drivetrain.getPose().toString())
        .withSize(3, 1)
        .withPosition(5, 2);

    diagnosticsTab
        .addString("Vision Estimated Pose:", () -> drivetrain.getVisionEstimatedPose().toString())
        .withSize(3, 1)
        .withPosition(5, 3);
  }
  ...
  public Command returnAutoCommand() {
    return drivetrain.followTrajectory(
        PathPlanner.loadPath("Test Path", new PathConstraints(2.5, 2.5)));
  }
    ...
    return new SequentialCommandGroup(
        ...
        new ParallelCommandGroup(
            new PPSwerveControllerCommand(
                ...
                new PIDController(7.5, 0, 0),
                // Rotation controller. Tune these values for your robot. Leaving them 0 will only
                // use feedforwards.
  ...
  // Attempting to pass auto into a separate command has not worked, even with the exact same
  // arguments
  ...
  private Command TestAuto2() {
    ...
    return new SequentialCommandGroup(new PathPlannerCommand(path, drivetrain, true).configure());
  }

  // Initial test of just driving on to the charging station and running auto balance
  private Command TestAutoBalance() {
    PathPlannerTrajectory path =
        PathPlanner.loadPath("AutoBalanceTest", new PathConstraints(2.5, 2.5));
    return new SequentialCommandGroup(
        new InstantCommand(() -> drivetrain.resetPosition(path.getInitialHolonomicPose())),
        new ParallelCommandGroup(
            new PPSwerveControllerCommand(
                ...
                new PIDController(
                    7.5, 0,
                    0), // X controller. Tune these values for your robot. Leaving them 0 will only
                // use feedforwards.
                new PIDController(
                    7.5, 0, 0), // Y controller (usually the same values as X controller)
                new PIDController(
                    0.5, 0,
  ...
  private Command C_Charge() {
    ...
    /*
     * return new SequentialCommandGroup( new ProxyCommand(() -> arm.moveTo(-75)).withTimeout(1),
     * new InstantCommand(() -> endEffector.releaseCube()), new WaitCommand(1), new
     * InstantCommand(() -> endEffector.stop()), new ParallelDeadlineGroup(
     * drivetrain.followTrajectory(path), //arm.moveTo(ArmConstants.stowLevel) just removed for
     * sketchy testing new ProxyCommand(() -> arm.moveTo(-120)).withTimeout(1) ), new
     * PrintCommand("Does this finish these commands?") new AutoBalance(drivetrain)
     *
     * <p>);
     */
    return new SequentialCommandGroup(
  ...
  // Moves from center grid onto and over the charge station, and then moves directly back onto the
  // charge station to auto balance
  private Command C_Cross_Charge() {
    PathPlannerTrajectory path =
        PathPlanner.loadPath("C-Cross-Charge", new PathConstraints(1.5, 1.5));
    return new SequentialCommandGroup(
        /*
         * new PrintCommand("\n\n" + path.getEndState().toString() + "\n\n"),
         * arm.moveTo(ArmConstants.lowLevel), new InstantCommand(() -> endEffector.releaseCube()),
         * new WaitCommand(0.5), new InstantCommand(() -> endEffector.stop()), new
         * ParallelDeadlineGroup( drivetrain.followTrajectory(path),
  ...
  // Places a high cone Moves from center grid onto and over the charge station, and then moves
  // directly back onto the charge station to auto balance
  private Command C_Cone_Cross_Charge() {
    ...
    return new SequentialCommandGroup(
        ...
        new ParallelCommandGroup(
            ...
            new SequentialCommandGroup(
                new WaitCommand(1), drivetrain.followTrajectory(path.get(0)))),
        new ParallelCommandGroup(
            arm.moveToAuto(-5).withTimeout(1),
            new SequentialCommandGroup(
                new WaitCommand(0.7), new InstantCommand(() -> endEffector.runIntakeAuto(-0.3)))),
        new ParallelCommandGroup(
            new SequentialCommandGroup(
  ...
  private Command C_Cone_Charge() {
    ...
    return new SequentialCommandGroup(
        ...
        new ParallelCommandGroup(
            new SequentialCommandGroup(drivetrain.followTrajectory(path.get(1))),
            new SequentialCommandGroup(
                arm.moveToAuto(ArmConstants.highLevel).withTimeout(0.5),
                new InstantCommand(() -> endEffector.stop()),
  ...
  private Command C_Cube_Cross_Charge() {
    ...
    return new SequentialCommandGroup(
        ...
        new ParallelCommandGroup(
            new SequentialCommandGroup(
                drivetrain.followTrajectory(path.get(1)), drivetrain.followTrajectory(path.get(2))),
            new SequentialCommandGroup(
                arm.moveToAuto(ArmConstants.highLevel).withTimeout(0.5),
                new InstantCommand(() -> endEffector.stop()),
                arm.moveToAuto(ArmConstants.stowLevel).withTimeout(3))),
  ...
  private Command OuterConeGrab() {
    ...
    return new SequentialCommandGroup(
        ...
        new ParallelCommandGroup(
            ...
            new SequentialCommandGroup(
                new WaitCommand(
                    0.25), // might need to decrease this time to get the arm down fast enough
                arm.moveToAuto(ArmConstants.pickup)
                    .withTimeout(3) // check whether the arm or path is completed faster
                )),
    ...
    return new SequentialCommandGroup(
        ...
        new ParallelCommandGroup(
            ...
            new SequentialCommandGroup(
                ...
                arm.moveToAuto(ArmConstants.highLevel)
                    .withTimeout(3) // check whether the arm or path is completed faster
                ),
            new SequentialCommandGroup(
                new WaitCommand(2.5), new InstantCommand(() -> endEffector.stop()))),
    ...
    return new SequentialCommandGroup(
        ...
        new ParallelCommandGroup(
            ...
            new SequentialCommandGroup(
                new WaitCommand(1), // if we up the speed, this may need to decrease a little bit
                arm.moveToAuto(ArmConstants.highLevel)
                    .withTimeout(2.5) // check whether the arm or path is completed faster
                ),
            new SequentialCommandGroup(
                new WaitCommand(2.5), new InstantCommand(() -> endEffector.stop()))),
        new InstantCommand(() -> endEffector.runIntake(0.8)),
  ...
  private Command LowLink() {
    ...
    return new SequentialCommandGroup(
        ...
        new ParallelCommandGroup(
            drivetrain.followTrajectory(path.get(1)),
            new SequentialCommandGroup(
                new ProxyCommand(() -> arm.moveToAuto(-70))
                    .withTimeout(
                        0.75), // arm angle may need to rise, need to check with inflated cubes
                new InstantCommand(() -> endEffector.stop()),
                new WaitCommand(1.6),
  ...
  private Command InnerCones() {
    ...
    return new SequentialCommandGroup(
        ...
        new ParallelCommandGroup(
            drivetrain.followTrajectory(path.get(1)),
            new SequentialCommandGroup(
                new WaitCommand(0.5), arm.moveToAuto(ArmConstants.stowLevel).withTimeout(2.5)),
            endEffector.turnWristToOffset(-10).withTimeout(1)),
        new ParallelCommandGroup(
            new InstantCommand(() -> endEffector.intakeCube()),
            drivetrain.followTrajectory(path.get(2)),
            new SequentialCommandGroup(
                new WaitCommand(2.3), arm.moveToAuto(ArmConstants.pickup).withTimeout(1))),
        new WaitCommand(1),
        new InstantCommand(() -> endEffector.stop()),
        new ParallelCommandGroup(
            new SequentialCommandGroup(
                ...
                arm.moveToAuto(ArmConstants.highLevel).withTimeout(2.5)),
            new SequentialCommandGroup(
                new WaitCommand(0.5), drivetrain.followTrajectory(path.get(3)))),
        new InstantCommand(() -> endEffector.runIntakeAuto(-0.5)));
  }

  // Moves from center grid, to the first cube, picks it up, and then moves to the outer grid to
  // score it.
  private Command C_1_O() {
    List<PathPlannerTrajectory> path =
        PathPlanner.loadPathGroup("C-1-O", new PathConstraints(1, 1), new PathConstraints(1, 1));
    return new SequentialCommandGroup(
        arm.moveTo(ArmConstants.lowLevel),
        new InstantCommand(() -> endEffector.releaseCube()),
        new WaitCommand(0.5),
        new InstantCommand(() -> endEffector.stop()),
        new ParallelDeadlineGroup(drivetrain.followTrajectory(path.get(0))),
        new InstantCommand(() -> endEffector.intakeCube()),
        new WaitCommand(0.5),
        new InstantCommand(() -> endEffector.stop()),
        new ParallelDeadlineGroup(drivetrain.followTrajectory(path.get(1))),
        new InstantCommand(() -> endEffector.releaseCube()),
        new WaitCommand(0.5),
        new InstantCommand(() -> endEffector.stop()));
  }

  // Moves from center grid, to the second cube, picks it up, and then moves to the outer grid to
  // score it.
  private Command C_2_O() {
    PathPlannerTrajectory path = PathPlanner.loadPath("C-2-O", new PathConstraints(1, 1));
    return new SequentialCommandGroup(drivetrain.followTrajectory(path));
  }

  // Moves from center grid, to the third cube, picks it up, and then moves to the inner grid to
  // score it.
  ...
  // Moves from center grid, to the fourth cube, picks it up, and then moves to the inner grid to
  // score it.
  ...
  // Moves from the center grid, towards the inner grid, and then proceeds to go all the way around,
  // drives up onto the charge station and auto balances.
  private Command C_InnerRoute_Charge() {
    PathPlannerTrajectory path =
        PathPlanner.loadPath("C-InnerRoute-Charge", new PathConstraints(1, 1));
    return new SequentialCommandGroup(
        drivetrain.followTrajectory(path), new AutoBalance(drivetrain));
  }

  // Moves from the center grid, towards the outer grid, and then proceeds to go all the way around,
  // drives up onto the charge station and auto balances.
  private Command C_OuterRoute_Charge() {
    PathPlannerTrajectory path =
        PathPlanner.loadPath("C-OuterRoute-Charge", new PathConstraints(1.5, 1.5));
    return new SequentialCommandGroup(
        drivetrain.followTrajectory(path), new AutoBalance(drivetrain));
  }

  // Moves from inner grid, to the third cube, picks it up, and then moves back to the inner grid to
  // score it.
  private Command I_3_I() {
    List<PathPlannerTrajectory> path =
        PathPlanner.loadPathGroup("I-3-I", new PathConstraints(2, 2), new PathConstraints(2, 2));
    return new SequentialCommandGroup(
        new ParallelDeadlineGroup(
            new SequentialCommandGroup(
                drivetrain.followTrajectory(path.get(0)), new WaitCommand(1)),
        ...
        new ParallelDeadlineGroup(
            ...
            drivetrain.followTrajectory(path.get(1)),
            new SequentialCommandGroup(
                new WaitCommand(2), new ProxyCommand(() -> arm.moveTo(ArmConstants.midLevel)))),
  ...
  private Command I_4_I() {
    List<PathPlannerTrajectory> path =
        PathPlanner.loadPathGroup(
            "I-4-I", new PathConstraints(2.5, 2.5), new PathConstraints(2.5, 2.5));
    return new SequentialCommandGroup(
        new ParallelDeadlineGroup(
            new InstantCommand(() -> endEffector.resetWristAngle()),
            new SequentialCommandGroup(
                drivetrain.followTrajectory(path.get(0)), new WaitCommand(2)),
        ...
        new ParallelDeadlineGroup(
            ...
            drivetrain.followTrajectory(path.get(1)),
            new SequentialCommandGroup(
                new WaitCommand(0.5), new ProxyCommand(() -> arm.moveTo(ArmConstants.midLevel)))),
        // new AutoMoveToAprilTag(drivetrain, drivetrain.getCamera()).withTimeout(1),
        new InstantCommand(() -> endEffector.releaseCube()),
        new WaitCommand(0.5),
        new InstantCommand(() -> endEffector.stop()));
  ...
  // Moves from the inner grid, goes around the side, and then moves up onto the charge station to
  // auto balance
  ...
  // Moves from outer grid, to the first cube, picks it up, and then moves back to the outer grid to
  // score it.
  private Command O_1_O() {
    // PathPlannerTrajectory path = PathPlanner.loadPath("O-1-O", new PathConstraints(1,1));
    List<PathPlannerTrajectory> path =
        PathPlanner.loadPathGroup("O-1-O", new PathConstraints(2, 2), new PathConstraints(2, 2));
    return new SequentialCommandGroup(
        new ParallelDeadlineGroup(
            new SequentialCommandGroup(
                new InstantCommand(() -> endEffector.resetWristAngle()),
            ...
            new SequentialCommandGroup(new ProxyCommand(() -> arm.moveTo(ArmConstants.pickup)))),
        new ParallelDeadlineGroup(
            new WaitCommand(5),
            drivetrain.followTrajectory(path.get(1)),
            new SequentialCommandGroup(
        ...
        // new ProxyCommand(() -> new AutoMoveToAprilTag(drivetrain,
        // drivetrain.getCamera()).withTimeout(1)),
        new InstantCommand(() -> endEffector.releaseCube()),
        new WaitCommand(0.5),
        new InstantCommand(() -> endEffector.stop()));
  }

  // Moves from outer grid, to the second cube, picks it up, and then moves back to the outer grid
  // to score it.
  ...
  // Moves from the outer grid, goes around the side, then goes onto the charge station to auto
  // balance
