...
public class AutoMoveToAprilTag extends CommandBase {
    ...
    if (m_camera.hasTargets()) {
      ...
      trajectoryToTag =
          PathPlanner.generatePath(
              ...
              new PathPoint(robotPosition, robotRotation),
              new PathPoint(
                  new Translation2d(robotPosition.getX() + 0.1, robotPosition.getY()),
                  robotRotation));
    }
  ...
  public void initialize() {
    /**
     * if (m_camera.hasTargets()) { robotPose = m_swerve.getPose(); robotPosition = new
     * Translation2d(robotPose.getX(), robotPose.getY()); robotRotation = robotPose.getRotation();
     * camToTarget = m_camera.getBestTarget().getBestCameraToTarget();
     * System.out.println(camToTarget.toString()); targetPose = robotPose.plus( new Transform2d( new
     * Translation2d(-camToTarget.getX() + 1, -camToTarget.getY()), new
     * Rotation2d(camToTarget.getRotation().getZ()) ) ); trajectoryToTag = PathPlanner.generatePath(
     * new PathConstraints(1, 1), new PathPoint(robotPosition, robotRotation), new
     * PathPoint(targetPose.getTranslation(), targetPose.getRotation()) );
     * System.out.println(trajectoryToTag.getInitialState().toString());
     * System.out.println(trajectoryToTag.getEndState().toString()); }
     */
  }
